"use client";

import React, { useState, useEffect, useMemo } from "react";
import { Category } from "@/types/product";
import { Search, X, Plus, Filter, ArrowUpDown, Info, Tag } from "lucide-react";
import { Button } from "@/components/common/Button";
import { CustomSelect, OptionItem } from "@/components/common/CustomSelect";
import { useDebounce } from "@/hooks/useDebounce";

interface ProductFiltersProps {
  search: string;
  category: string;
  sortOption: string;
  pageSize: number;
  categories: Category[];
  isCategoriesLoading?: boolean;
  onSearchChange: (search: string) => void;
  onCategoryChange: (category: string) => void;
  onSortChange: (sortOption: string) => void;
  onPageSizeChange: (size: number) => void;
  onAddProduct: () => void;
  onResetFilters: () => void;
}

export function ProductFilters({
  search,
  category,
  sortOption,
  pageSize,
  categories,
  isCategoriesLoading = false,
  onSearchChange,
  onCategoryChange,
  onSortChange,
  onPageSizeChange,
  onAddProduct,
  onResetFilters,
}: ProductFiltersProps) {
  const [searchInput, setSearchInput] = useState(search);
  const debouncedSearch = useDebounce(searchInput, 400);

  useEffect(() => {
    setSearchInput(search);
  }, [search]);

  useEffect(() => {
    if (debouncedSearch !== search) {
      onSearchChange(debouncedSearch);
    }
  }, [debouncedSearch, search, onSearchChange]);

  const categoryOptions: OptionItem[] = useMemo(() => {
    return [
      {
        value: "",
        label: "All Categories",
        icon: <Filter className="w-3.5 h-3.5 text-blue-500" />,
      },
      ...categories.map((cat) => ({
        value: cat.slug,
        label: cat.name,
        icon: <Tag className="w-3.5 h-3.5 text-slate-400" />,
      })),
    ];
  }, [categories]);

  const sortOptions: OptionItem[] = [
    {
      value: "",
      label: "Default Order",
      description: "Default API rank",
    },
    {
      value: "price-asc",
      label: "Price: Low to High",
      description: "Cheapest first",
    },
    {
      value: "price-desc",
      label: "Price: High to Low",
      description: "Premium first",
    },
    {
      value: "rating-desc",
      label: "Rating: High to Low",
      description: "Top reviewed first",
    },
    {
      value: "rating-asc",
      label: "Rating: Low to High",
      description: "Lowest rated first",
    },
    {
      value: "title-asc",
      label: "Title: A to Z",
      description: "Alphabetical",
    },
    {
      value: "title-desc",
      label: "Title: Z to A",
      description: "Reverse alphabetical",
    },
  ];

  const pageSizeOptions: OptionItem[] = [
    { value: 10, label: "10 per page" },
    { value: 20, label: "20 per page" },
    { value: 50, label: "50 per page" },
  ];

  const hasActiveFilters = !!search || !!category || !!sortOption;

  return (
    <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-4 sm:p-5 mb-6 space-y-4">
      {/* Search and Add Button */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
          <input
            type="text"
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
            placeholder="Search products by title, keywords, brand..."
            className="w-full pl-10 pr-10 py-2.5 text-sm font-medium rounded-2xl border border-slate-200 bg-slate-50/70 placeholder:text-slate-400 focus:bg-white focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10 transition-all"
          />
          {searchInput && (
            <button
              onClick={() => {
                setSearchInput("");
                onSearchChange("");
              }}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-slate-100 transition cursor-pointer"
              title="Clear search"
            >
              <X className="w-4 h-4" />
            </button>
          )}
        </div>

        <Button
          variant="primary"
          onClick={onAddProduct}
          leftIcon={<Plus className="w-4 h-4" />}
          className="shrink-0 rounded-2xl shadow-md shadow-blue-600/20 font-bold"
        >
          Add Product
        </Button>
      </div>

      {/* Custom Dropdown Filters Row */}
      <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100">
        <div className="flex flex-wrap items-center gap-3">
          {/* Custom Category Dropdown */}
          <CustomSelect
            options={categoryOptions}
            value={category}
            onChange={(val) => onCategoryChange(val)}
            icon={<Filter className="w-3.5 h-3.5 text-blue-600" />}
            disabled={isCategoriesLoading}
            placeholder="All Categories"
            dropdownClassName="w-56"
          />

          {/* Custom Sort Dropdown */}
          <CustomSelect
            options={sortOptions}
            value={sortOption}
            onChange={(val) => onSortChange(val)}
            icon={<ArrowUpDown className="w-3.5 h-3.5 text-indigo-600" />}
            placeholder="Default Sort"
            dropdownClassName="w-52"
          />

          {/* Clear Filters Button */}
          {hasActiveFilters && (
            <button
              onClick={() => {
                setSearchInput("");
                onResetFilters();
              }}
              className="text-xs font-bold text-rose-600 hover:text-white bg-rose-50 hover:bg-rose-600 px-3 py-2 rounded-2xl transition-all shadow-2xs flex items-center gap-1 cursor-pointer"
            >
              <X className="w-3.5 h-3.5" /> Clear Filters
            </button>
          )}
        </div>

        {/* Custom Page Size Dropdown */}
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
          <span>Show:</span>
          <CustomSelect
            options={pageSizeOptions}
            value={pageSize}
            onChange={(val) => onPageSizeChange(Number(val))}
            align="right"
            dropdownClassName="w-36"
          />
        </div>
      </div>

      {search && category && (
        <div className="flex items-center gap-2 bg-blue-50/80 border border-blue-200/70 text-blue-900 text-xs px-3.5 py-2.5 rounded-2xl animate-fadeIn">
          <Info className="w-4 h-4 text-blue-600 shrink-0" />
          <span>
            <strong>Search & Category active:</strong> DummyJSON API performs global search; results are filtered locally by category.
          </span>
        </div>
      )}
    </div>
  );
}
